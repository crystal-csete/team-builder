# Module Project: Forms - teams Builder
...
In this project you'll build an app that will keep track of memebers of a teams. You'll be able to add members to this teams as well as edit their details.
...
- [ ] Add your teams lead as collaborator on Github.
- [ ] Clone your OWN version of the repository in your terminal
- [ ] Use `npx create-react-app teams-builder --use-npm` or `create-react-app teams-builder` to initiate the project.
- [ ] CD into your `teams-builder` directory
...
- [ ] Render a list of teams members from state.
- [ ] Build a form to add a new member to the list of members.

#### Setup your state

- [ ] Import the `useState` hook and set up state to keep your teams members list.
- [ ] Give the state variable you just declared a default value. You will need to keep track of a list of teams members and each teams member will have several key/value pairs associated with them.
- [ ] Render your list of teams members.
...
- [ ] Render your `Form` component in `App`. The `App` component should hand down through props any callback(s) needed for `Form` to do its job (adding new members to your teams members state on submit).
...
Next we're going to reuse the `Form` component to edit teams members. You can try to accomplish this goal completely on your own, or you can follow the steps below. These steps assume your `Form` maintains its own state.
...
1. Our user clicks the `Edit` button/icon to start editing a teams member
...
This is an interesting bit of architecture we've done so far. We have built a reusable form that can be used to add a teams member, or edit a teams member. The last piece of the puzzle is this - when we submit the form, do we run an `addMember` function, or an `editMember` function? And how will the form know? Well, our form knows if we are editing a teams member by whether or not there is the prop `memberToEdit`. (P.S. This can also be done with a boolean - something like `isEditing`...)

Okay, now that we understand how `Form.js` knows if we are editing or creating members, we can finish this app!

- [ ] Create a new function in `App.js` called `editMember` that will edit one or more of the details of a teams member and pass it down to the second form component. You'll have to find a way to loop over the data in your state, find the specific teams member you intend to edit, and finally change the details with the data sent up to you from the second form component. `.map()` will be your friend for that task. You'll also want to avoid directly mutating your data. The `...` spread operator will be your friend there.
...
- [ ] Build another layer of your App so that you can keep track of multiple teamss, each with their own encapsulated list of teams members.
- [ ] Look into the various strategies around form validation. What happens if you try to enter a number as a teams-members name? Does your App allow for that? Should it? What happens if you try and enter a function as the value to one of your fields? How could this be dangerous? How might you prevent it?
